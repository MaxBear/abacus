...
class Supervisor:
    ...
    async def _finish(self, handled: Handled) -> None:
        ...
        match ending:
            case Ending.STALE:
                # Nothing *written*, on purpose. The row belongs to whoever took
                # the lease, the artifact was never made, and every write this
                # worker could attempt would either raise `StaleLease` or, worse,
                # land on a job someone else is midway through.
                #
                # `discard` is not an exception to that — it decides nothing
                # about the job. It hands back what the queue is still holding
                # for a claim that is void, which on RabbitMQ is an unacked
                # delivery and at `prefetch=1` is this worker's only slot.
                # Without it this is the one ending that settles nothing: every
                # other path leaves through `ack` or `nack`, both of which
                # release the delivery in a `finally` even when the row refuses
                # them. A worker that lost a lease race would go on reserving,
                # be offered nothing ever again, and look idle rather than
                # broken — `test_losing_the_lease_race_does_not_cost_a_consumer
                # _its_prefetch_slot` is the same wedge reached from the `ack`
                # side, and the fix there cannot see this path because this path
                # is the one that calls neither.
                log.warning("job %s: discarding a solve whose lease is void", lease.job.id)
                await self._queue.discard(lease)
