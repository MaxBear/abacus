...
def solving(ready_file: Path):
    """Wait for the *child*, not for the row.

    `running` goes true at `reserve`, which is before `spawn` has even exec'd a
    new interpreter. A test that kills on that signal is signalling a bootstrap:
    the solver has not installed `SIG_IGN` yet, so the child dies to the SIGTERM
    and the assertion on SIGKILL fails — or, worse, passes on a fast machine and
    fails on a loaded one. The solver touches this file once it is genuinely
    running, so waiting on it is the difference between proving layer 3 and
    proving layer 2 twice.
    """
...
async def test_a_stale_lease_mid_solve_kills_the_child_and_installs_no_pointer(
    ...
    ready = tmp_path / "solving"
    job = await enqueue(queue, burn_seconds=FOREVER, ignore_sigterm=True, ready_file=str(ready))

    turn = asyncio.ensure_future(worker.run_once())
    await until(solving(ready), what="the child to start ignoring SIGTERM")
    ...
    # But it did hand its own claim back. `STALE` is the only ending that
    # reports nothing, which makes it the only one that would otherwise settle
    # nothing: every other path leaves through `ack` or `nack`, and both release
    # the delivery in a `finally` even when the row refuses them. On RabbitMQ at
    # `prefetch=1` the delivery this frees is the loser's only slot.
    assert queue.discarded == [result.lease.id]
    assert result.lease.id != thief.id
...
async def test_cancelling_a_solve_inside_a_long_call_still_ends_the_job(queue, store, tmp_path):
