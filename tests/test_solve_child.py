...
# that. It matters in both signalling tests, for the same underlying reason:
# a SIGTERM delivered mid-`spawn` says nothing about the solver. The child that
# installs `SIG_IGN` has to have installed it before the signal arrives, or the
# test proves layer 2 twice and layer 3 never; the child with no opinion has to
# actually be running the solver, or it dies inside the bootstrap and the exit
# status is not reliably SIGTERM at all.
...
# What a prompt SIGKILL costs on top of the grace: signal the child, reap it,
# and read the outcome back. Milliseconds on an idle machine, so this is nearly
# all headroom for a loaded one. It exists so `grace + REAP_SLACK` is an upper
# bound that says "the kill followed the grace immediately" rather than a
# number chosen to be comfortably large.
REAP_SLACK = timedelta(seconds=2)
...
# The escalation bound has to be able to fail before `PATIENT` rescues it —
# otherwise the test that checks the kill was prompt can only ever see the hang.
assert BRIEF_GRACE + REAP_SLACK < PATIENT
...
async def test_sigterm_stops_a_solver_that_has_no_opinion_about_it():
    ...
    # Signal a solver, not a half-built interpreter. Without this the SIGTERM
    # lands during `spawn`'s bootstrap and the child comes back killed by
    # SIGKILL as often as by SIGTERM — reproducible with no project code at all,
    # by `p.start()` immediately followed by `p.terminate()`.
...
async def test_a_solver_that_ignores_sigterm_is_killed_anyway():
    ...
    # second timeout stacked on the first. A `stop` that waited on anything
    # further out — `PATIENT`, most plausibly — would still satisfy every
    # assertion above and only fail here.
    assert elapsed < (BRIEF_GRACE + REAP_SLACK).total_seconds()
